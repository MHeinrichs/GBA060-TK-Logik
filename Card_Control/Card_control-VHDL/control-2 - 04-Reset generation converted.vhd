...
architecture control_behav of control is
   signal CAQ0_CLK_ctrl,
	 ...
	 QDSACK0_CLK, QDSACK0_ACLR, STOPHALT, STOPRES, BYTEPORT, WORDPORT, LONGPORT, AMISEL, NAMIACC,
	 ATERM, LEND, CAQ0, CAQ1, CAQ2, CNTDIS, QDSACK1, QDSACK0, LDSACK1,
	 LDSACK0, AMIQ1, AMIQ0, RSTINT: std_logic;
   signal 
	 LE_BS_FB, DS30_FB, RSTINT_FB, AMIQ0_FB, AMIQ1_FB,
...
-- Start of original equations
   ...
   IPL40(0) <= (RSTINT and IPL30(0)) or ((not RSTINT) and '1');
   IPL40(1) <= (RSTINT and IPL30(1)) or ((not RSTINT) and '1');
   IPL40(2) <= (RSTINT and IPL30(2)) or ((not RSTINT) and '1');
   CDIS40 <= RSTINT or ((not RSTINT) and '1');
   MDIS40 <= RSTINT or ((not RSTINT) and '1');
   BGR60 <= '0';
   RW30 <= RW40;
   A30_LE <= '0';
   ICACHE <= ((not TT40(1)) and (not TM40(2)) and TM40(1) and (not TM40(0))) or
	 ((not TT40(1)) and TM40(2) and TM40(1) and (not TM40(0)));
   FC30(0) <= ((not TT40(1)) and (not TM40(1))) or ((not TT40(1)) and TM40(1) and
	 TM40(0)) or (TT40(1) and (not TT40(0)) and TM40(0)) or (TT40(1) and
	 TT40(0));
   FC30(1) <= ((not TT40(1)) and TM40(1) and (not TM40(0))) or (TT40(1) and (not
	 TT40(0)) and TM40(1)) or (TT40(1) and TT40(0));
   FC30(2) <= ((not TT40(1)) and TM40(2) and (not TM40(1)) and TM40(0)) or ((not
	 TT40(1)) and TM40(2) and TM40(1) and (not TM40(0))) or (TT40(1) and (not
	 TT40(0)) and TM40(2)) or (TT40(1) and TT40(0));
   QDSACK0_ACLR <= NAMIACC;
   QDSACK0_CLK <= SCLK_SIG;
   QDSACK0_D <= (not DSACK30(0)) or (not STERM30);
   QDSACK1_ACLR <= NAMIACC;
   QDSACK1_CLK <= SCLK_SIG;
   QDSACK1_D <= (not DSACK30(1)) or (not STERM30);
   LDSACK0_ACLR <= NAMIACC;
   LDSACK0_CLK <= not SCLK_SIG;
   LDSACK0_D <= ((not DSACK30(0)) and (QDSACK0 or QDSACK1)) or ((not STERM30)
	 and (QDSACK0 or QDSACK1)) or (LDSACK0 and AMIQ1 and AMIQ0 and (not
	 CNTDIS));
   LDSACK1_ACLR <= NAMIACC;
   LDSACK1_CLK <= not SCLK_SIG;
   LDSACK1_D <= ((not DSACK30(1)) and (QDSACK0 or QDSACK1)) or ((not STERM30)
	 and (QDSACK0 or QDSACK1)) or (LDSACK1 and AMIQ1 and AMIQ0 and (not
	 CNTDIS));
   OE_BS <= AMISEL;
   DIR_BS <= RW40;
   LONGPORT <= (RSTI40_SIG and LDSACK1 and LDSACK0) or (LONGPORT and (not LEND));
   WORDPORT <= (RSTI40_SIG and LDSACK1 and (not LDSACK0)) or (WORDPORT and (not
	 ((not CAQ2) and (not CAQ1) and (not CAQ0))));
   BYTEPORT <= (RSTI40_SIG and (not LDSACK1) and LDSACK0) or (BYTEPORT and (not
	 ((not CAQ2) and (not CAQ1) and (not CAQ0))));
   TA40_OE <= AMISEL;
   ...
   process (CAQ0_FB, CAQ1_FB, CAQ2_FB, TS40, SEL16M, LONGPORT, A40(0), WORDPORT,
	 AMISEL, RW40, BYTEPORT, RSTI40_SIG, ATERM, LDSACK1, LDSACK0, CNTDIS,
   ...
   begin
	 ...
	 BWL_BS(2) <= not ((AMISEL and (not RW40)) or (AMISEL and RW40 and
	       BYTEPORT));
	 if (RSTI40_SIG and ATERM and (not LDSACK1) and LDSACK0 and (not CNTDIS)
	 ...
	 elsif (RSTI40_SIG and ATERM and (not LDSACK1) and LDSACK0 and (not CNTDIS)
	       and SIZ40(1) and (not SIZ40(0)) and A40(1))='1' then
	    (CAQ2_D, CAQ1_D, CAQ0_D) <= std_logic_vector'("100");
	 elsif (RSTI40_SIG and ATERM and LDSACK1 and (not LDSACK0) and (not CNTDIS)
	       and (((not SIZ40(1)) and (not SIZ40(0))) or (SIZ40(1) and
	       SIZ40(0))))='1' then
	    (CAQ2_D, CAQ1_D, CAQ0_D) <= std_logic_vector'("011");
	 elsif ((RSTI40_SIG and ATERM and LDSACK1 and LDSACK0 and (not CNTDIS)) or
	       (RSTI40_SIG and ATERM and LDSACK1 and (not LDSACK0) and (not CNTDIS)
	       and ((SIZ40(1) and (not SIZ40(0))) or ((not SIZ40(1)) and
	       SIZ40(0)))) or (RSTI40_SIG and ATERM and (not LDSACK1) and LDSACK0
